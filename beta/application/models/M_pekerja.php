...
// ------------------------- Master User
  ...
  public function get_list_kandidat_num($keywrd=null,$lokasi=null,$spsliz=null,$jobpos=0,$typjob=0,$salmin=0,$salmax=0)
  {
    $sql = "
      ...
      INNER JOIN t_mas_common d ON a.PKR_RELIGN = d.COM_TYPECD AND d.COM_HEADCD = 1
      WHERE PKR_ACTIVE = 1
    ";
  ...
  public function get_list_kandidat($limit, $start, $keywrd=null,$lokasi=null, $spsliz=null,$jobpos=0,$typjob=0,$salmin=0,$salmax=0)
  {
    ...
    $sql = "
      ...
      e.COM_DESCRE JNSKLM_DESC, d.COM_DESCRE RELIGION, f.COM_DESCRE EDUCATION, g.COM_DESCRE MARRIED
      FROM T_MAS_PEKRJA a
      INNER JOIN t_mas_provnc b ON a.PKR_CONTRY = b.PRV_CONTRY AND a.PKR_PROVNC = b.PRV_IDENTS
      INNER JOIN t_mas_cityss c ON a.PKR_CONTRY = c.CTY_CONTRY AND a.PKR_PROVNC = c.CTY_PROVNC AND a.PKR_CITYSS = c.CTY_IDENTS
      INNER JOIN t_mas_common d ON a.PKR_RELIGN = d.COM_TYPECD AND d.COM_HEADCD = 1
      INNER JOIN t_mas_common e ON a.PKR_JNSKLM = e.COM_TYPECD AND e.COM_HEADCD = 2
      INNER JOIN t_mas_common f ON a.PKR_EDUCID = f.COM_TYPECD AND f.COM_HEADCD = 3
      INNER JOIN t_mas_common g ON a.PKR_MARRID = g.COM_TYPECD AND g.COM_HEADCD = 14
      WHERE PKR_ACTIVE = 1
    ";
