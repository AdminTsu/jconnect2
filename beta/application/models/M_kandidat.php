...
// ------------------------- Master Job
  function getJobList($level,$loginid){
    ...
    $this->db->select("
      CASE 
        WHEN JOB_STATUS = 1 THEN 'AKTIF'
        WHEN JOB_STATUS = 2 THEN 'SELEKSI' 
        WHEN JOB_STATUS = 3 THEN 'BATAL' 
        WHEN JOB_STATUS = 4 THEN 'TUTUP' 
        WHEN JOB_STATUS = 5 THEN 'EXPIRED' 
        ELSE 'SELESAI' 
      END STATUS
    ");
  ...
  function getDataCompany($id){
    $this->db->select('CLI_IDENTS,CLI_NAMESS,CTY_NAMESS CITY,CASE WHEN a.CLI_ASPECS = 15 THEN a.CLI_BDLAIN ELSE SPS_NAMESS END ASPECS');
  ...
  public function get_list_kandidat_num($keywrd=null,$lokasi=null,$spsliz=null,$jobpos=0,$typjob=0,$salmin=0,$salmax=0)
  {
    $sql = "
      SELECT count(*) as jml
      FROM T_MAS_PEKRJA a
      INNER JOIN t_mas_provnc b ON a.PKR_CONTRY = b.PRV_CONTRY AND a.PKR_PROVNC = b.PRV_IDENTS
      INNER JOIN t_mas_cityss c ON a.PKR_CONTRY = c.CTY_CONTRY AND a.PKR_PROVNC = c.CTY_PROVNC AND a.PKR_CITYSS = c.CTY_IDENTS
      WHERE PKR_ACTIVE = 1
    ";
    ...
    if ($salmin != '' && $salmax == '') {
      $sql .= " AND JOB_SALMIN >= '$salmin'";
    }

    if ($salmin == '' && $salmax != '') {
      $sql .= " AND JOB_SALMAX <= '$salmax'";
    }

    if ($salmin != '' && $salmax != '') {
      $sql .= " AND (JOB_SALMIN >= '$salmin' AND JOB_SALMAX <= '$salmax')";
    }
  ...
  public function get_list_kandidat($limit, $start, $keywrd=null,$lokasi=null, $spsliz=null,$jobpos=0,$typjob=0,$salmin=0,$salmax=0)
  {
    // echo "$keywrd ~ $lokasi <br>";
    $sql = "
      SELECT a.*, case when PKR_CONTRY = 1 then 'Indonesia' else 'Jepang' end DESC_CONTRY, b.PRV_NAMESS DESC_PROVNC, c.CTY_NAMESS DESC_CITYSS,
      CASE WHEN PKR_JNSKLM = 1 then 'LAKI-LAKI' else 'PEREMPUAN' END JNSKLM_DESC
      FROM T_MAS_PEKRJA a
      INNER JOIN t_mas_provnc b ON a.PKR_CONTRY = b.PRV_CONTRY AND a.PKR_PROVNC = b.PRV_IDENTS
      INNER JOIN t_mas_cityss c ON a.PKR_CONTRY = c.CTY_CONTRY AND a.PKR_PROVNC = c.CTY_PROVNC AND a.PKR_CITYSS = c.CTY_IDENTS
      WHERE PKR_ACTIVE = 1
  ...
  public function get_list_bykandidat_num($idclient)
  {
    $sql = "
      SELECT count(*) as jml
      FROM T_TRN_JOBPOS a
  ...
  public function get_list_bykandidat($limit, $start, $idclient,$jml)
  {
    // echo "$keywrd ~ $lokasi <br>";
    if($jml > 0){
      $sql = "
        SELECT distinct *
        FROM T_TRN_JOBPOS a
        RIGHT JOIN T_MAS_CLIENT b ON a.JOB_COMPNY = b.CLI_IDENTS
        WHERE JOB_STATUS = 1
        AND JOB_COMPNY = ".$idclient."
        ORDER BY JOB_NOMJOB
        LIMIT ".$start.",".$limit."
  ...
  function getKandidatUpdate_Detail($id){
    $this->db->select("*,TIMEDIFF(NOW(),PKR_DATCRT) JAM,DATEDIFF(NOW(),PKR_DATCRT) HARI,case when PKR_CONTRY = 1 then 'Indonesia' else 'Jepang' end DESC_CONTRY");
    $this->db->select("b.PRV_NAMESS DESC_PROVNC, c.CTY_NAMESS DESC_CITYSS, e.COM_DESCRE JNSKLM_DESC, d.COM_DESCRE RELIGION, f.COM_DESCRE EDUCATION, g.COM_DESCRE MARRIED");
    $this->db->from('T_MAS_PEKRJA a');
    $this->db->join("T_MAS_PROVNC b","a.PKR_CONTRY = b.PRV_CONTRY AND a.PKR_PROVNC = b.PRV_IDENTS","INNER");
    $this->db->join("T_MAS_CITYSS c","a.PKR_CONTRY = c.CTY_CONTRY AND a.PKR_PROVNC = c.CTY_PROVNC AND a.PKR_CITYSS = c.CTY_IDENTS","INNER");
    $this->db->join("T_MAS_COMMON d","a.PKR_RELIGN = d.COM_TYPECD AND d.COM_HEADCD = 1","INNER");
    $this->db->join("T_MAS_COMMON e","a.PKR_JNSKLM = e.COM_TYPECD AND e.COM_HEADCD = 2");
    $this->db->join("T_MAS_COMMON f","a.PKR_EDUCID = f.COM_TYPECD AND f.COM_HEADCD = 3");
    $this->db->join("T_MAS_COMMON g","a.PKR_MARRID = g.COM_TYPECD AND g.COM_HEADCD = 14","INNER");
    $this->db->where("PKR_ACTIVE",1);
    $this->db->where("PKR_IDENTS",$id);
    $this->db->order_by("PKR_DATCRT");
