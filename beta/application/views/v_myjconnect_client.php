...
$optCountry = array('0'=>'--Pilih Negara--','1'=>'Indonesia');
...
$optJeniss = array('0'=>'--Pilih Data--','1'=>'Kontrak','2'=>'Tetap');
    ...
    foreach($data as $row){
        // if($type==1){
            ...
            $idbidang = isset($row['CLI_ASPECS']) ? $row['CLI_ASPECS'] : NULL;
...
$(document).ready(function(){
    ...
    $('#txtPASWD2').bind('change',function(data){
        ...
        if(pass1 != pass2){
            alert('Kata sandi yang Anda masukkan tidak sama, harap masukan kembali kata sandi Anda!');
            $('#txtPASWD2').val('');
            $('#txtPASWD2').focus();
            return;
        }
            ...
            <div class="col-xl-3 col-lg-3 col-md-4">
                ...
                <div class="job-category-listing mb-50" id='side_myjconnect'>
                        <div class="single-listing">
                            ...
                            <div class="single-listing">
                                <a id='menu_web' href="<?php echo base_url(); ?>myjconnect_client"><i class="fa fa-building"></i>&nbsp;&nbsp;Profile Perusahaan</a>
                            </div><br>
                            <div class="single-listing">
...
<!-- ----------------------------------------------------  view profile ------------------------------------------------------------- -->
            <table class='table1'>
                    ...
                    <td rowspan=5>
                        <center>
                            ...
                            <?php if($logoss != '' || $logoss != null){?>
                                <img style='height: 150px;border: 1px solid gainsboro;border-radius:3px;border-color:#f2f2f2;' src="<?=base_url()?>assets/documents/upload/client/<?=$idclnt?>/<?=$logoss?>" alt="Avatar" class="image" style="width:100%">
                            <?php }else{?>
                                <img style='height: 150px;border: 1px solid gainsboro;border-radius:3px;border-color:#f2f2f2;' src="<?=base_url()?>resources/img/no-image2.jpg" alt="Avatar" class="image">
                            <?php }?>
                                <div class="middle">
                                    </div>
                                </div>
                ...
                <tr><td style="top: 0;">No. Fax</td><td colspan=3>: <?=$faxnom?></td></tr>
...
<?php
    ...
    if($type == 6){
        $optJENISS = array('0'=>'Pilih Data','1'=>'Rekap','2'=>'Group By Job Type');
        $optBulan = array(
            '0'=>'Pilih Bulan',
            '01'=>'Januari',
            '02'=>'Februari',
...
function jvCetak(jenis){
    ...
    if(jenis != 0){
        $.post('/Myjconnect_Client/cetakLaporan/'+types+'/'+tahun+'/'+bulan+'/'+jenis,param,function(data){
            window.open('/Myjconnect_Client/cetakLaporan/'+types+'/'+tahun+'/'+bulan+'/'+jenis,'newwindow','width=1040,height=860');
            window.close();
            hideLoading();
        });
    }
    ...
    //     });
        ...
        // html += "<input type='text' name='types' id='param' value='"+types+"'/>";
        // html += "<input type='text' name='years' id='param' value='"+tahun+"'/>";
        // html += "<input type='text' name='bulan' id='param' value='"+bulan+"'/>";
        // html += "<input type='text' name='jenis' id='param' value='"+jenis+"'/>";
