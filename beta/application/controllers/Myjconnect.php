...
var $userid;
    ...
    function index() {
        ...
        $data['detail'] = $this->m_jconnect->getDetailPengalaman($this->userid);
        $data['jml'] = $this->m_jconnect->getDetailPengalaman_count($this->userid);
    ...
    function profile_edit($id) {
        ...
        $data['title'] = 'My JConnect';
        $data['content'] = 'myjconnect';
        $data['menu'] = 'Ubah Profile';
        $data['icon'] = '<i class="fa fa-user"></i>';
        $data['type'] = 1;
        $data['jenis']= 'edit';
        $data['data'] = $this->m_jconnect->getKandidatInfo($this->userid);
        $data['detail'] = $this->m_jconnect->getDetailPengalaman($id);
        $data['jml'] = $this->m_jconnect->getDetailPengalaman_count($id);
    ...
    function profile_simpan($id){
        ...
        $PKR_IDENTS = $id;
        $PKR_CODESS = $this->input->post('nomors');
        $PKR_NAMESS = ucwords($this->input->post('namess'));
        $PKR_FOTOSS = $this->input->post('fotoss');
        $PKR_TMPLHR = ucfirst($this->input->post('birthp'));
        $PKR_TGLLHR = $this->input->post('birthd');
        $PKR_EMAILS = $this->input->post('emails');
        $PKR_CONTRY = $this->input->post('contry');
        $PKR_PROVNC = $this->input->post('provnc');
        $PKR_CITYSS = $this->input->post('cityss');
        $PKR_ADDRES = ucfirst($this->input->post('addres'));
        $PKR_ADDRDO = ucfirst($this->input->post('addrdo'));
        $PKR_NOMKTP = str_replace("_","",str_replace("-","",str_replace(".","",$this->input->post('nomktp'))));
        $PKR_NONPWP = str_replace("_","",str_replace("-","",str_replace(".","",$this->input->post('nonpwp'))));
        $PKR_NOMTLP = str_replace("_","",str_replace("-","",str_replace(".","",$this->input->post('nomtlp'))));
        $PKR_NOMRHP = str_replace("_","",str_replace("-","",str_replace(".","",$this->input->post('nomrhp'))));
        $PKR_JNSKLM = $this->input->post('jnsklm');
        $PKR_RELIGN = $this->input->post('relign');
        $PKR_EDUCID = $this->input->post('eductn');

        $PKR_MARRID = $this->input->post('marrid');
        $PKR_EXPNPT = strtoupper($this->input->post('expnpt'));
        $PKR_EXPBDG = ucfirst($this->input->post('posisi'));
        $PKR_EXPRNC = $this->input->post('exprnc');
        $PKR_DESCRE = $this->input->post('deskjb');

        $PKR_VISAFL = $this->input->post('visafl');
        $PKR_LVLBHS = $this->input->post('lvlbhs');
        $PKR_SSWFIL = $this->input->post('sswfil');
        $PKR_MAGANG = $this->input->post('magang');
        $PKR_MNTBDG = $this->input->post('mntbdg');
        $PKR_BDLAIN = ucfirst($this->input->post('bdlain'));
        $PKR_EXPSAL = $this->input->post('expsal');
        $PKR_FILECV = $this->input->post('filecv');
        $PKR_ACTIVE = $this->input->post('active');

        // echo "$PKR_IDENTS - $PKR_NAMESS - $PKR_TMPLHR - $PKR_TGLLHR - $PKR_JNSKLM - $PKR_RELIGN - $PKR_EDUCID - $PKR_NOMKTP";die();
        ...
        // --------------------------------------------- update table Detail
        if(count($DETAIL) > 0){
            // $this->crud->useTable("T_MAS_PEKRJA_EXPRNC",false);
            // $this->crud->delete_builder(array("EXP_IDNPKR"=>$IDENTS));
            // $this->crud->unsetMe();
            ...
            // foreach ($DETAIL) {
                ...
                $value = array(
                    'EXP_IDNPKR'=>$IDENTS,
                    'EXP_PTNAME'=>$DETAIL['expnam'],
                    'EXP_BIDANG'=>$DETAIL['expbdg'],
                    'EXP_POSISI'=>$DETAIL['posisi'],
                    'EXP_EXPRNC'=>$DETAIL['explen'],
                    'EXP_JBDESK'=>$DETAIL['expdes']
                );

                $this->crud->useTable("T_MAS_PEKRJA_EXPRNC");
...
// -----------------------------------------------------  Pengalaman Kerja dan Sertifikasi ---------------------------------------- //
    function pengalaman() {
        ...
        $data['title'] = 'My JConnect';
        $data['content'] = 'myjconnect';
        $data['menu'] = 'Pengalaman dan Sertifikasi';
    ...
    function pengalaman_edit($id) {
        ...
        $data['title'] = 'My JConnect';
        $data['content'] = 'myjconnect';
        $data['menu'] = 'Ubah Pengalaman dan Sertifikasi';
        $data['icon'] = '<i class="fa fa-briefcase"></i>';
        $data['type'] = 2;
        $data['jenis']= 'edit';
        $data['data'] = $this->m_jconnect->getKandidatInfo($this->userid);
...
// -----------------------------------------------------  Minat Kerja ---------------------------------------- //
    ...
    function minat_kerja_simpan($id){
        ...
        $redirect = '/Myjconnect/minat_kerja';
...
// -----------------------------------------------------  Lamaran ---------------------------------------- //
    ...
    // function upload_foto($IDENTS){
    //     $fotoss = $this->input->post('txtFOTOSS');
    //     echo $fotoss;die();
    //     //awal aplod file RND
    //     $path = PATH_PEKERJA_PHOTO.$IDENTS.'/';
    //     if( is_dir($path) === false )
    ...
    //         mkdir($path);  
    //     }
    //     // echo $_FILES['txtFOTOSS']['name'];die();
    ...
    //         move_uploaded_file($_FILES['txtFOTOSS']['tmp_name'], $path.$file_image_name);
    //         // echo $file_image_name;
    //         $data2 = array_merge($data2,array('PKR_FOTOSS'=>$file_image_name));
    //         $this->crud->useTable('T_MAS_PEKRJA');
    //         $this->crud->save($data2, array('PKR_IDENTS'=> $IDENTS));
    //         $this->crud->unsetMe();
