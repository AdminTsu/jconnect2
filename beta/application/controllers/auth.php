...
class Auth extends CI_Controller 
{
	...
	function test(){
		$connect = ldap_connect("kemas.co.id", 389);
		if(ldap_bind($connect, "dicky@kemas.co.id", "splasshboyz")){
			echo "succes";
		}
		// echo $connect;		
	}
	public function loginad(){
   	$adServer = "ldap://kemas.co.id";
    ...
    $bind = @ldap_bind($ldap, $ldaprdn, $password);
        ...
        $result = ldap_search($ldap,"dc=kemas,dc=co, dc=id",$filter);
	...
	public function login()
	{
		...
		$username = 'aplikasi';//strtolower($this->input->post('username'));
		$password = 'kompak';//$this->input->post('password');
