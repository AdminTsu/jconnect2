...
var $userid;
    ...
    function profile_simpan($id){
        ...
        $input = array(
            ...
            'CLI_PHONES' => $CLI_NOMTLP,
            'CLI_MOBILE' => $CLI_NOMRHP,
    ...
    function lowongan_simpan(){
        ...
        $JOB_PSTION = $this->input->post('cmbJOBPOS');
        ...
        $this->crud->unsetMe();
        $IDENTS = ($this->input->post('hidTRNSKS') =='add') ? $this->crud->__insertID : $JOB_IDENTS;

        $redirect = '/myjconnect_client/lowongan/'.$JOB_COMPNY;
    ...
    function getDetailPerekrutan($id){
        $getdata = $this->m_perekrutan->getPerekrutanWeb_detail($id,2);
        $jmlrow = $getdata->num_rows();
        $i = 1;
        $namesnya = '';
        ...
        if($jmlrow != 0 || $jmlrow != ''){
            foreach($getdata->result_array() as $row){
                if($row['PKR_NAMESS'] != $namesnya){
                    $dataDetail .= "
                        <tr style='border: 1px solid #000000;'>
                            <td style='text-align:center;border: 1px solid #000000;'>".$i."</td>
                            <td style='text-align:center;border: 1px solid #000000;'>".ucwords(strtolower($row['PKR_NAMESS']))."</td>
                            <td style='border: 1px solid #000000;'>".ucwords(strtolower($row['JNSKLM']))."</td>
                            <td style='text-align:left;border: 1px solid #000000;'>".ucwords(strtolower($row['PKR_EXPRNC']))."</td>
                            <td style='text-align:center;border: 1px solid #000000;'>".ucwords(strtolower($row['PKR_EXPNPT']))."</td>
                            <td style='text-align:center;border: 1px solid #000000;'>".ucwords(strtolower($row['PKR_EXPBDG']))."</td>
                        </tr>
                    ";
                    $namesnya = $row['PKR_NAMESS'];
                }
            }
        }else{
            $dataDetail .= "
                <tr style='border: 1px solid #000000;'>
                    <td colspan=6 style='text-align:center;border: 1px solid #000000;'>Belum ada data yang diinput.</td>
                </tr>
            ";
        }
    ...
    function comboNomorJob($IDJOBNYA=0){
        $listPekerja = '
                        <select name="cmbPEKRJA" id="cmbPEKRJA" class="form-control" onChange="jvChangePekerja()" required>
                            <option value=0>-- Pilih Pekerja --</option>
        ';

        if($IDJOBNYA > 0){
            $optPekerja = $this->m_jconnect->getPekerja_autotag($IDJOBNYA);
            foreach($optPekerja as $row){
                $listPekerja .= "<option value='".$row->PKR_IDENTS."'>".$row->PKR_NAMESS."</option>";
            }
        }
        ...
        ';
        $callback = array('listPekerja'=>$listPekerja);
        echo json_encode($callback);
    }
    ...
    function popup_detail() {
        ...
        // $nomjob     = $this->input->post('nomjob');
        $identsnya  = $this->input->post('idnya');
        ...
        // $callback = array('viewDetail'=>$html);
        // echo json_encode($callback);
        echo $html;
    }
    ...
    function cetakLaporan($types,$tahun,$bulan,$jenis){
        if($bulan > 0){
        ...
        $data['tahun'] = $tahun;
        $data['bulan'] = $bulan;
        $datanya = $this->m_client->getApplayJobReport($this->userid,$types,$tahun,$bulan);
        $data['data'] = $datanya;
        foreach($datanya as $row){
            $ASPECS = $row['JOB_ASPECS'];
            $data['aspecs'] = $ASPECS;
            $aspecsnya = $ASPECS;
        }
        ...
        if($types == 1){
            $judulnya = 'Rekap';
            $headsnya = '<span style=\'font-size: 7px;text-align:center;\'>Periode : '.$periode.'</span>';
        }else{
            $judulnya = 'Group by Job Type';
            $headsnya = '
                <span style=\'font-size: 7px;text-align:center;\'>
                    Periode : '.$periode.'<br>
                    Spesialisasi : '.$aspecsnya.'
                </span>
            ';
        }
        ...
        if($jenis == 1){
            ...
            header("Content-type: application/vnd-ms-excel");
            header("Content-Disposition: attachment; filename=Laporan-$judulnya-".date('Ymd-H:i:s').".xls");
            header("Expires: 0");
            ...
            $title = 'Laporan '.$judulnya;
            $paper = 'A4';
            $orientation = "portrait";
            $content = $this->load->view('v_rpt_html',$data, true);
            $image_file = '<img src="/resources/assets/img/logo/Logo_jconnect_new.png" alt="test alt attribute" width="60" height="30" border="0" />';

            $pdf = new Pdf('P', PDF_UNIT, 'A4', true, 'UTF-8', false);
            // set document information
            $pdf->SetCreator('JConnect');
            $pdf->SetAuthor('JConnect');
            $pdf->SetTitle($title);
            $pdf->SetSubject('Laporan JConnect');
            ...
            $html .= "
                <center><p style=\"font-size: 12px;text-align:center;\"><b><u>".$title."</u></b></p><center>
                <p>
                    ".$headsnya."<br><br>
                    <table style=\"border: 1px solid black;border-collapse: collapse;font-size: 9px;\" width=\"100%\">
                        <tr style=\"margin: 5px\">
                            <th class=\"kepala\" style=\"border:1px solid black;width:5%;\">No</th>
                            <th class=\"kepala\" style=\"border:1px solid black;width:8%;\">No. Jobs</th>
                            <th class=\"kepala\" style=\"border:1px solid black;width:25%;\">Lowongan</th>
                            <th class=\"kepala\" style=\"border:1px solid black;width:20%;\">Nama</th>
                            <th class=\"kepala\" style=\"border:1px solid black;width:20%;\">Tempat/Tgl.Lahir</th>
                            <th class=\"kepala\" style=\"border:1px solid black;width:12%;\">Jenis Kelamin</th>
                            <th class=\"kepala\" style=\"border:1px solid black;width:10%;\">No. HP</th>
                        </tr>
            ";
            ...
            foreach($datanya as $row){
                ...
                $titles = $row['TITLES'];
                $pekrja = $row['PEKRJA'];
                $tmtglh = $row['TMPTGLLHR'];
                $jnsklm = $row['JNSKLM'];
                $nomrhp = $row['NOMRHP'];

                $html .= '
                    <tr>
                        <td style="border:1px solid black;font-size: 6px;" align="center">'.$i.'</td>
                        <td style="border:1px solid black;font-size: 6px;" align="center">'.$nomjob.'</td>
                        <td style="border:1px solid black;font-size: 6px;" align="left">'.$titles.'</td>
                        <td style="border:1px solid black;font-size: 6px;" align="left">'.$pekrja.'</td>
                        <td style="border:1px solid black;font-size: 6px;" align="left">'.$tmtglh.'</td>
                        <td style="border:1px solid black;font-size: 6px;" align="left">'.$jnsklm.'</td>
                        <td style="border:1px solid black;font-size: 6px;" align="right">'.$nomrhp.'</td>
                    </tr>
                ';
    ...
    function exportExcel(){
        ...
        $arrExcel = array('sNAMESS'=>'Laporan Excel', 'sFILNAM'=>'Laporan Excel '.$datenow,'col'=>$arrCol, 'rsl'=>$jdata->result());
