...
class Profile extends MY_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model(array('m_common','m_master','m_pekerja','m_client','m_jconnect'));
    ...
    function vinfo_rekanan()
    {
        ...
        if($txtLOGOSS == ''){
            ...
            $txtLOGOSS = 'upload/client/'.$idnya.'/'.$txtLOGOSS;
        }

        $optASPECS1 = $this->m_client->getComboBidang();
        $optASPECS2 = array('99'=>'BIDANG LAINNYA');
        $optASPECS = array_merge($optASPECS1, $optASPECS2);
        $optActive = $this->crud->getCommon(3,6);
        ...
        $arrTable[] = array('group'=>1, 'urutan'=>$urutan++, 'type'=> 'hid', 'namanya' =>'txtLOGINS', 'label'=>'Login ID/Username', 'size'=>'300','value'=>isset($column) ? $txtLOGINS : "",'tagsinput' => $AccountTag,'readonly'=>$readonly);
        $arrTable[] = array('group'=>1, 'urutan'=>$urutan++, 'type'=> 'txt', 'namanya' =>'txtNAMESS','label'=>'Nama Client',  'size'=>'300','value'=> isset($column) ? $txtNAMESS : "");
        $arrTable[] = array('group'=>1, 'urutan'=>$urutan++, 'type'=> 'cmb', 'namanya' =>'cmbASPECS', 'size'=>'250','label'=>'Bidang Pekerjaan','option'=>$optASPECS, 'value'=> isset($column) ? (isset($status) ? $cmbASPECS_DESC : $cmbASPECS) : 0); 

        $arrTable[] = array('group'=>1, 'urutan'=>$urutan++, 'type'=> 'txt', 'namanya' =>'txtBDLAIN','label'=>'Bidang Lain',  'size'=>'300','value'=> isset($column) ? $txtBDLAIN : "");
        $arrTable[] = array('group'=>1, 'urutan'=>$urutan++, 'type'=> 'txt', 'namanya' =>'txtWEBSIT','label'=>'Website',  'size'=>'300','value'=> isset($column) ? $txtWEBSIT : "");
        ...
        $arrTable[] = array('group'=>1, 'urutan'=>$urutan++, 'type'=> 'txt', 'namanya' =>'txtCONTAC','label'=>'Nama Admin',  'size'=>'300','value'=> isset($column) ? $txtCONTAC : "",'placeHolder'=>'Bapak/Ibu');
        ...
        $arrTable[] = array('group'=>1, 'urutan'=>$urutan++, 'type'=> 'hid', 'namanya' =>'hidIDENTS','label'=>'IDENTS', 'value'=> $idnya);
        $arrTable[] = array('group'=>1, 'urutan'=>$urutan++, 'type'=> 'hid', 'namanya' =>'hidLOGNID','label'=>'IDENTS', 'value'=> $param);
    ...
    function save_rekanan(){
        ...
        $CLI_WEBSIT = str_replace("http://","",$this->input->post('txtWEBSIT'));
