...
class M_Jconnect extends CI_Model{
	...
	function getClientID($id){			
  	...
  	function cekAccount($table,$usrnam){
	    $this->db->select('count(USR_LOGINS) HASIL');
	    $this->db->from($table);
	    $this->db->where("((USR_LOGINS = '$usrnam') OR (USR_EMAILS = '$usrnam'))");
  	...
  	function cekDataAkun($table,$usrnam){
	    $this->db->select('*');
	    $this->db->from($table);
	    $this->db->where("((USR_LOGINS = '$usrnam') OR (USR_EMAILS = '$usrnam'))");
	...
	function getDetailPengalaman($id){
		$this->db->select("EXP_IDENTS,EXP_PTNAME, EXP_BIDANG, EXP_POSISI, EXP_JBDESK, 
							CONCAT(DATE_FORMAT(EXP_DATMSK, '%M'),' ',DATE_FORMAT(EXP_DATMSK, '%Y')) MSKKRJ,
							CONCAT(DATE_FORMAT(EXP_DATAKH, '%M'),' ',DATE_FORMAT(EXP_DATAKH, '%Y')) KLRKRJ,
							CASE 
								WHEN EXP_EXPRNC = 1 THEN '1-2 Tahun'
								WHEN EXP_EXPRNC = 1 THEN '2-3 Tahun'
								WHEN EXP_EXPRNC = 1 THEN '3-6 Tahun'
								WHEN EXP_EXPRNC = 1 THEN '> 6 Tahun'
							END EXP_EXPRNC,EXP_SALARY
		");
		$this->db->from("T_MAS_PEKRJA_EXPRNC a");
		$this->db->join("T_MAS_PEKRJA b","a.EXP_IDNPKR = b.PKR_IDENTS","INNER");
		$this->db->where("(b.PKR_IDENTS = ".$id." OR b.PKR_LOGNID = ".$id.")");
	...
	function getDetailPengalaman_edit($id){
		$this->db->select("a.*");
		$this->db->from("T_MAS_PEKRJA_EXPRNC a");
		$this->db->join("T_MAS_PEKRJA b","a.EXP_IDNPKR = b.PKR_IDENTS","INNER");
		$this->db->where("(b.PKR_IDENTS = ".$id." OR b.PKR_LOGNID = ".$id.")");
	...
	function getDetailPengalaman_count($id){
  	...
  	function cekAccountToken($type,$table,$account,$token){
	    $this->db->select('COUNT(TKN_ACCONT) HASIL');
	    $this->db->from($table);
	    if($type == 1){
	    	$this->db->where("TKN_ACCONT = '$account'");
	    }else{
	    	$this->db->where("TKN_ACCONT = '$account' AND TKN_TOKENS = '$token'");
	    }
  	...
  	function GetCountAccount($table,$account,$token){
	    $this->db->select('tkn_counts+1 jml');
	    $this->db->from('T_MAS_TOKEN');
	    $this->db->where("TKN_ACCONT = '$account'");
